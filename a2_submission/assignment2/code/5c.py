...
for ind_chain in range(n_chains):
    ...
    row = ind_chain / 10
    col = ind_chain % 10
    this_array = np.asarray(v_sample.reshape(np.sqrt(n_visible), np.sqrt(n_visible)))
...
f.subplots_adjust()
# plt.show()
f.savefig("../plot/5c_"+str(k)+"_" + sys.argv[2].split("/")[-1] + "_" + method + ".png")
