...
class Model_feed_foward:
    ...
    def load_pretrained_weight(self,epoch_name):

        candidate_model_filename = [file for file in os.listdir("../dump/") if epoch_name in file and "model" in file][0]
        pre_trained_model = pickle.load(open("../dump/" + candidate_model_filename, "r"))
        if type(pre_trained_model.W) == type([]):
            pre_trained_model.W = pre_trained_model.W[1]

        # raw_input("loaded pre-trained model shape "+str(pre_trained_model.W.shape)+" where it should replaced into ? "+str(self.W[1].shape))

        self.W[1]=pre_trained_model.W
...
if __name__ == "__main__":
    def plot_curve(label=""):
        ...
        pickle.dump(plot_epoch_ce_train, open("../dump/a1_train" + leading_label + "_ce_" + label, "w"))
        pickle.dump(plot_epoch_ce_valid, open("../dump/a1_valid" + leading_label + "_ce_" + label, "w"))
        pickle.dump(plot_epoch_ce_test, open("../dump/a1_test" + leading_label + "_ce_" + label, "w"))
        ...
        pickle.dump(plot_epoch_cr_train, open("../dump/a1_train" + leading_label + "_cr_" + label, "w"))
        pickle.dump(plot_epoch_cr_valid, open("../dump/a1_valid" + leading_label + "_cr_" + label, "w"))
        pickle.dump(plot_epoch_cr_test, open("../dump/a1_test" + leading_label + "_cr_" + label, "w"))

        pickle.dump(model, open("../dump/a1_model_" + leading_label + "_" + label, "w"))
        pickle.dump(speed, open("../dump/a1_speed_" + leading_label + "_" + label, "w"))
    ...
    plot_epoch_cr_test = []
    speed = []
    model = Model_feed_foward(args.num_layer, [num_dimension, args.hidden_layer_1_dimension, args.num_class], args.lr, args.lbd,
                  args.momentum, args.batch_normalization)


    if not args.pre_trained_model=="None" and not len(args.pre_trained_model)==0:   # model epoch here
        print "loading pre-trained model weight here! "
        model.load_pretrained_weight(args.pre_trained_model)



    if not args.hidden_layer_2_dimension == 0:
        args.num_layer = 2
        model = Model_feed_foward(args.num_layer,
